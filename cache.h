...
private:
    ...
    ******************************************/
    ...
    int m_reSizingPhase = 0; //are we moving the first 25%, 2nd, 3rd or 4th?
    int m_quarterPush = 0; //how many we need to move per
    int m_currentIndex = 0; //the current index to pick up from when transferring

    //return index where a certain person should be inserted
    int findIndex(Person person);

    //re-size the table and transfer
    void reSize();

    //copy of insert() without any resizing
    void altInsert(Person person);

    //copy of findIndex() that only looks at the current Table
    int altFindIndex(Person person);
};
#endif
